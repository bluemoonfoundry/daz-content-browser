...
2. ~~Whether Studio 6's headers differ from Studio 4.5's for these specific classes~~ — checked (beads-2mw.10): the class APIs Subsystem B actually calls are almost entirely compatible (one non-breaking `insertController` return-type widening `void`→`bool`, and `DzMorph::getValueChannel()` still present but flagged `// TODO : SDK 6 : remove` in favor of a new `getValueControl()`). The real incompatibility is at the **build layer**, not the header layer: `cpp/cmake/DazStudioSDK.cmake` assumes SDK 4.5's directory layout (`lib/<plat>/dzcore.lib`) and vendored-Qt-inside-SDK model, neither of which SDK6 uses (`<plat>/lib/dzcore.lib`, and Qt6 must be supplied externally via `find_package(Qt6)` + a user `Qt6_DIR`). Tracked as a separate, scoped task: beads-31u.
...
- `DAZ_SDK_DIR` is a required CMake cache variable pointing at a locally-installed Daz Studio SDK — the SDK is proprietary and is never vendored into this repo (matches how this project already treats other external, non-redistributable dependencies). Four SDK variants already exist locally for development/testing, at `y:/working/BlueMoonFoundry/daz-studio-sdks/{daz-studio-45-sdk,daz-studio-6-sdk}-{win32,macos}`; this design was investigated against `daz-studio-45-sdk-win32` specifically (its headers ship pre-extracted-able from `daz_sdk_headers.zip` in that directory). `daz_plugin`'s CMake accepts `DAZ_SDK_DIR` pointing at either `daz-studio-45-sdk-win32` variant without separate build logic. **Studio 6 needs real (not hypothetical) separate build logic** (beads-2mw.10, tracked as beads-31u): its directory layout and Qt6-must-be-supplied-externally model are incompatible with the current `DazStudioSDK.cmake`, which was written for SDK 4.5's conventions. No `#ifdef`s are needed in `injector_core`/`daz_plugin`'s own C++ source for this, though — the class-level API differences audited in beads-2mw.10 are minor and non-breaking; this is purely a CMake/build-configuration gap.
